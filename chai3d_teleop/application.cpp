...
void updateHaptics(void)
{
    ...
    while (simulationRunning)
    {
        ...
        if (mode.compare("Teleop_Uni") == 0 || mode.compare("Teleop_Bi_Pos") == 0 || mode.compare("Teleop_Bi_Force") == 0 || mode.compare("Teleop_Bi_1:1") == 0) {
            if (isFirstTimeSetup) 
            {
            ...
            if (mode.compare("Teleop_Bi_1:1") == 0) 
            {
                ...
                force = kpm * (positionFollower - position) + kdm * (linearVelocityMaster - linearVelocityFollower);
